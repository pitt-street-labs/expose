...
The signed JSON file produced by a single FatFinger6000 pipeline run, conforming to the schema in `schemas/canonical-artifact-v1.json`. Sole deliverable of the system.
...
**Cosign**
The Sigstore project's signing tool for container images and arbitrary blobs. FatFinger6000 uses cosign keyless signing via GitHub Actions OIDC for production artifacts.

**CTEM (Continuous Threat Exposure Management)**
The discipline of continuously identifying, prioritizing, and addressing exposures in an organization's attack surface. FatFinger6000 produces input feeds for CTEM platforms.
...
The discovery and inventory of an organization's externally reachable assets. FatFinger6000 is, fundamentally, an EASM tool whose deliverable is structured for CTEM consumption.
...
**Environment 1**
The FatFinger6000 pipeline itself. Performs deterministic discovery, collection, sanitization, attribution, bounded LLM enrichment, and artifact generation. Subject of this specification.

**Environment 2**
The downstream operational environment that consumes FatFinger6000 artifacts. Performs open-ended narrative reasoning, red team lead briefings, and exploit hypothesis generation. May use Mythos-class capabilities under appropriate safeguards. Air-gapped from Environment 1 via manual JSON handoff.
...
Anthropic's coalition program providing Claude Mythos Preview access to selected technology companies and critical-infrastructure organizations. Out of scope for FatFinger6000 itself but relevant context for Environment 2.

**Helm chart**
The deployment artifact for FatFinger6000. Single chart deploys to ARC, AWS, Azure, GCP, or customer-on-prem with different values files.
...
**Tenant**
A logical isolation boundary in FatFinger6000. v1 ships with a single `default` tenant. Multi-tenant deployments serve N tenants with independent configuration, data, and artifacts.
...
**Trust boundary**
Architectural boundary across which content is treated as untrusted. FatFinger6000 has two: between collection (Stage 2) and sanitization (Stage 3), and between deterministic graph state (Stage 4a) and LLM context (Stage 4b).
