...
| Engine source code | `src/expose/` — 140+ Python source files across 18 sub-packages: `api/` (admin, credentials, events, export, findings, graph, run_log, runs, tenants, tenant_config), `broker/`, `cli.py`, `collectors/` (framework + 29 builtin incl. ct-certspotter, common-crawl, otx-alienvault, git-commit-emails, paste-monitor, mail-headers, sip-discovery, wikipedia-edits, dns-chaos, security-txt), `compliance/`, `crypto/`, `db/`, `egress/` (direct + socks5 + ip_guard SSRF protection), `eval/`, `import_/`, `llm/`, `maintenance/`, `observability/`, `pipeline/` (dispatcher + executor + seed expansion + entity-seed converter + credential resolver + enforcement + enrichment + target_profile + collector_filter + supply_chain + provider_fingerprints), `quotas/`, `repositories/`, `sanitization/`, `scope/`, `secrets/` (memory_backend with file persistence), `storage/`, `types/`, `ui/` |
...
| Tests | `tests/` — **3200+ tests as of f1db9b0.** 90+ test files. Conftest provides `pg_container` + `nats_container` shared session fixtures. aiosqlite for fast API unit tests. 1 pre-existing failure tracked in #73. |
