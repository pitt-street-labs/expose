...
| Tests | `tests/` — **5000+ tests as of f77a369.** 160+ test files. |

## Issue tracker conventions

- **181 closed / 12 open / 193 total.** Quality session closed #189-196 (8 issues). v1-tagged: all closed. All original high-priority issues closed. All critical issues closed. Moat restructuring issues #183–#188 closed 2026-05-12.
...
- **Production deployment v0.2.2 (2026-05-12):** Attribution scoring fix + BGP fix + credentials loaded (8 global + 8 tenant keys) + Gemini LLM enrichment working. First successful production scan: 826 entities, 45 relationships for korlogos.com (3 passes).
- **Quality session (2026-05-12):** ISP contamination fix (582 optimum.com entities → 0), registrar supply chain scoring (GoDaddy: +30 points), attribution inheritance (subdomains inherit parent's tier), CISO findings (internal hostname leakage, no-HTTPS), CT retry logic (3x with backoff), email delivery API (`POST .../reports/ciso/deliver`), report justifications (context-specific text). Container image tag in Quadlet: `localhost/expose:0.4.1`.
