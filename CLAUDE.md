...
| Engine source code (Sprint 1+) | `src/expose/` — `__init__.py`, `cli.py` (click-based skeleton; W4.A converts to package), `types/` (Pydantic models mirroring schemas/), `db/` (SQLAlchemy ORM + Alembic-backed schema), `collectors/` (framework + base ABC + tier gating), `sanitization/` (Sprint 3 done), **`broker/`** (NATS JetStream client + Worker ABC + stream setup, W1.A), **`crypto/`** (FIPS-validated SHA-256 + cert fingerprint adapter, W1.B / ADR-010), **`repositories/`** (async tenant-scoped Entity/Relationship/Run repos, W1.C), **`maintenance/`** (incidental-data retention pruner, W1.E / closes #31), **`secrets/`** (SecretsBackend ABC + InMemoryBackend, W1.F), **`import_/`** (SpiderFootImporter + SPIDERFOOT_MODULE_MAP, W1.F / advances #8) |
...
| Tests | `tests/` — `test_smoke.py`, `test_schema_sync.py` (Pydantic-vs-schema round-trips), `test_tenant_isolation.py` (7 placeholders pending W3.B real impl, closes #28), `test_fips_crypto_gate.py` (banned-import scan + positive-case adapter tests per ADR-010), `test_collectors_framework.py`, plus Wave 1: `test_broker.py` (5 tests, NATS via testcontainers), `test_repositories.py` (8 integration), `test_retention_pruner.py` (8 integration), `test_secrets.py` (7), `test_spiderfoot_import.py` (8). Conftest provides `pg_container` + `nats_container` shared session fixtures. **123 unit + 20 integration passing as of 9b0ece4.** |

## Issue tracker conventions

- 37 open / 41 total issues. Closed: #36 (orchestration framework — NATS JetStream selected), #38 (Session H — EXPOSE selected), **#34 (ETHICS.md, closed Wave 1 W1.D)**, **#31 (incidental data retention pruning, closed Wave 1 W1.E)**.
- v1-tagged remaining: **#28 (cross-tenant isolation tests — foundation landed in W1.C, full suite in W3.B)** and **#3 (multi-arch container builds + cosign — Dockerfile + CI buildx already wired, W4.C verifies + writes keypair docs)**.
- Other in-flight: #40 (Sprint 1-2 foundation, in-progress), #41 (rulepack `$schema` bug, xfailed), #8 (secrets backends — advanced by W1.F SecretsBackend skeleton; production Vault/KMS impls deferred), #20 (SECURITY.md, advanced by W1.D), #28, #3, #39 (Tor/proxy egress design), plus 30 others.
- Labels follow `epic:<slug>`, `area:<slug>`, `priority:<level>`, `type:<kind>`, plus `v1` (now 2 remaining: #3, #28).
