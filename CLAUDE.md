...
| Engine source code (Sprint 1+) | `src/expose/` — `__init__.py`, `cli.py` (click CLI with `run start/status/list` commands + seed-type auto-detection, W4.A), `types/` (Pydantic models mirroring schemas/), `db/` (SQLAlchemy ORM + Alembic-backed schema), `collectors/` (framework + base ABC + tier gating + **5 builtin collectors**: ct-crtsh, cloud-ranges, rdap-whois, active-dns-resolve, active-http-fingerprint, W2), `sanitization/` (Sprint 3 done), `broker/` (NATS JetStream client + Worker ABC + stream setup, W1.A), `crypto/` (FIPS-validated SHA-256 + cert fingerprint adapter, W1.B / ADR-010), `repositories/` (async tenant-scoped Entity/Relationship/Run repos, W1.C), `pipeline/` (PipelineDispatcher + RunExecutor + seed expansion + contextvars tenant propagation, W3.A), `observability/` (OTel tracing + structlog logging + metrics instruments, W3.C), `maintenance/` (incidental-data retention pruner, W1.E / closes #31), `secrets/` (SecretsBackend ABC + InMemoryBackend, W1.F), `import_/` (SpiderFootImporter + SPIDERFOOT_MODULE_MAP, W1.F / advances #8) |
...
| Tests | `tests/` — **330 tests passing as of 9ff2af8 (Sprint 3-4 complete).** Core: `test_smoke.py`, `test_schema_sync.py`, `test_fips_crypto_gate.py`, `test_collectors_framework.py`. Wave 1: `test_broker.py`, `test_repositories.py`, `test_retention_pruner.py`, `test_secrets.py`, `test_spiderfoot_import.py`. Wave 2: `test_ct_crtsh.py` (25), `test_cloud_ranges.py` (16), `test_rdap_whois.py` (13), `test_active_dns.py` (16), `test_active_http.py` (19). Wave 3: `test_pipeline_dispatcher.py` (12), `test_run_executor.py` (15), `test_seed_expansion.py` (11), `test_observability.py` (10), `test_tenant_isolation.py` (9 real tests, **#28 closed**). Wave 4: `test_cli_run.py` (17), `test_cli_status.py` (8), `test_e2e_smoke.py` (3 integration). Conftest provides `pg_container` + `nats_container` shared session fixtures. |
| Cosign docs | `deploy/cosign-keypair-setup.md` (production signing path; lab unsigned per ADR-004) |

## Issue tracker conventions

- 35 open / 41 total issues. Closed: #36, #38, #34, #31, **#28 (cross-tenant isolation tests, closed Wave 3 W3.B)**, **#3 (multi-arch container builds + cosign docs, closed Wave 4 W4.C)**.
- v1-tagged: **all closed** (#3 and #28 were the last two).
- In-flight: #40 (Sprint 1-2 foundation, in-progress), #41 (rulepack `$schema` bug, xfailed), #8 (secrets backends — advanced by W1.F skeleton; production Vault/KMS deferred), #20 (SECURITY.md, advanced by W1.D), #39 (Tor/proxy egress design), plus 30 others.
