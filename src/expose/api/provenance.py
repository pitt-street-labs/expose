...
async def get_provenance(
    ...
    2. **Rules applied** — extracted from the ``_rules_applied`` list.
    3. **Relationships** — all edges (incoming and outgoing).
    ...
    """
    from expose.services.provenance_service import ProvenanceService  # noqa: PLC0415

    service = ProvenanceService(session)
    result = await service.get_provenance(tenant_id, entity_id)
